...
import { CservicesListFilterHeader } from "./servicesListFilterHeader";
import { PaginationState } from "@tanstack/react-table";
import { useQuery } from "react-query";
import { FilterservicesData, servicesListTableDataColumn } from "./columns";
import ReactPaginate from "react-paginate";
import "./styles.css";
import { useDispatch } from "react-redux";
import { fetchData, setSearcParam, setrefresh } from "./servicesslice";
import { useAppSelector } from "@/Redux/Hooks";
import Createservices from "./Createservices";
import Editservices from "./Editservices";
import Deleteservices from "./Deleteservices";

const CservicesListContainer = () => {
  const dispatch = useDispatch();
  const [filterText, setFilterText] = useState("");
  const { refresh, searcParam, servicesFilter } = useAppSelector(
          ...
          <Card>
            <CardBody>
              <div className="list-product-header">
                ...
                <Editservices />
                <Deleteservices />
                <CservicesListFilterHeader />

                <Collapse isOpen={servicesFilter}>
                  <Card className="shadow-none">
                    <CardBody className="list-product-body">
                      <Row className="row-cols-xl-5 row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-2 g-3">
                        {FilterservicesData.map((item, index) => (
                          <Col key={index}>
                            <Input
                              type="select"
                              onChange={(e) =>
              ...
              <div className="list-product">
                <div className="table-responsive">
                  <DataTable
                    ...
                    columns={servicesListTableDataColumn}
                    striped
                    highlightOnHover
                    subHeader
...
export default CservicesListContainer;
