...
import { deletelivraisons, editlivraisons, setDeleteModal, setlivraisonsValidation, setrefresh } from "./livraisonsslice";

const Deletelivraisons = () => {
  const { deletemodal,livraisonsValidation } = useAppSelector((state) => state.livraisons);
  const dispatch = useAppDispatch();
  const toggle = () => dispatch(setDeleteModal());
  const { formdata } = useAppSelector((state) => state.livraisons);
  const {register,formState: { errors },handleSubmit, reset} = useForm<livraisonsType>({
    values: formdata,
  });

  const Deletelivraisons: SubmitHandler<livraisonsType> = async (data) => {
    console.log(data);
    if (data !== undefined) {

      try {

        const result =  await dispatch(deletelivraisons(data));
    ...
    <>
      <Modal className="modal-bookmark" isOpen={deletemodal} toggle={toggle} size="lg">
        <div className="modal-header">
          <h3 className="modal-title">Supprimer livraisons</h3>
          <Button close color="transparent" onClick={toggle}></Button>
        </div>
        <ModalBody>
          <Form className="form-bookmark needs-validation" onSubmit={handleSubmit(Deletelivraisons)}>
            <Row className="g-2">
                                   <FormGroup>
            ...
            <Button color="secondary" className="me-1" type="submit" onClick={() => dispatch(setlivraisonsValidation(false))}>Supprimer</Button>
...
export default Deletelivraisons;
