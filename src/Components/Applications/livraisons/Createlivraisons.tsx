...
import { createlivraisons, setlivraisonsValidation, setModal, setrefresh } from "./livraisonsslice";

const Createlivraisons = () => {
  const { modal,livraisonsValidation } = useAppSelector((state) => state.livraisons);
  const {register,formState: { errors },handleSubmit, reset} = useForm<livraisonsType>();
  const dispatch = useAppDispatch();
  const toggle = () => dispatch(setModal());

  const AddContact: SubmitHandler<livraisonsType> = async (data) => {
    console.log(data);
    if (data !== undefined) {

      try {

        const result =  await dispatch(createlivraisons(data));
        dispatch(setModal());
        dispatch(setrefresh());
    ...
    <>
      <Modal className="modal-bookmark" isOpen={modal} toggle={toggle} size="lg">
        <div className="modal-header">
          <h3 className="modal-title">Ajouter livraisons</h3>
        ...
        <ModalBody>
          <Form className="form-bookmark needs-validation" onSubmit={handleSubmit(AddContact)}>
            ...
            <Button color="secondary" className="me-1" type="submit" onClick={() => dispatch(setlivraisonsValidation(true))}>Enregistrer</Button>
...
export default Createlivraisons;
