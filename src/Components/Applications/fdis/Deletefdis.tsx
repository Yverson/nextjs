...
const Deletefdis = () => {
  const { deletemodal, fdisValidation } = useAppSelector((state) => state.fdis);
  const dispatch = useAppDispatch();
  const toggle = () => dispatch(setDeleteModal());
  const { formdata } = useAppSelector((state) => state.fdis);
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const Deletefdis: SubmitHandler<fdisType> = async (data) => {
    console.log(data);
    if (data !== undefined) {
      try {
        const result = await dispatch(deletefdis(data));
    ...
    <>
      ...
      >
        <div className="modal-header">
          <h3 className="modal-title">Supprimer fdis</h3>
          <Button close color="transparent" onClick={toggle}></Button>
        </div>
        <ModalBody>
          <Form
            className="form-bookmark needs-validation"
            onSubmit={handleSubmit(Deletefdis)}
          >
            <Row className="g-3">
            ...
            <Button
              ...
              className="me-1"
              type="submit"
              onClick={() => dispatch(setfdisValidation(false))}
            >
              Supprimer
            </Button>
