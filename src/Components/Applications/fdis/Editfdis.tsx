...
const Editfdis = () => {
  const { editmodal, fdisValidation } = useAppSelector((state) => state.fdis);
  const dispatch = useAppDispatch();
  const toggle = () => dispatch(setEditModal());
  const { tempId, formdata } = useAppSelector((state) => state.fdis);
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const Editfdis: SubmitHandler<fdisType> = async (data) => {
    console.log(data);
    if (data !== undefined) {
      try {
        const result = await dispatch(editfdis(data));
      ...
      >
        ...
        <ModalBody>
          <Form
            ...
            onSubmit={handleSubmit(Editfdis)}
          >
            <Row className="g-3">
