...
import { deletesuivie_douanes, editsuivie_douanes, setDeleteModal, setsuivie_douanesValidation, setrefresh } from "./suivie_douanesslice";

const Deletesuivie_douanes = () => {
  const { deletemodal,suivie_douanesValidation } = useAppSelector((state) => state.suivie_douanes);
  const dispatch = useAppDispatch();
  const toggle = () => dispatch(setDeleteModal());
  const { formdata } = useAppSelector((state) => state.suivie_douanes);
  const {register,formState: { errors },handleSubmit, reset} = useForm<suivie_douanesType>({
    values: formdata,
  });

  const Deletesuivie_douanes: SubmitHandler<suivie_douanesType> = async (data) => {
    console.log(data);
    if (data !== undefined) {

      try {

        const result =  await dispatch(deletesuivie_douanes(data));
    ...
    <>
      <Modal className="modal-bookmark" isOpen={deletemodal} toggle={toggle} size="lg">
        <div className="modal-header">
          <h3 className="modal-title">Supprimer suivie_douanes</h3>
          <Button close color="transparent" onClick={toggle}></Button>
        </div>
        <ModalBody>
          <Form className="form-bookmark needs-validation" onSubmit={handleSubmit(Deletesuivie_douanes)}>
            <Row className="g-2">
                                   <FormGroup>
            ...
            <Button color="secondary" className="me-1" type="submit" onClick={() => dispatch(setsuivie_douanesValidation(false))}>Supprimer</Button>
...
export default Deletesuivie_douanes;
