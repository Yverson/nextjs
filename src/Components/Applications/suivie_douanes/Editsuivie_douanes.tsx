...
import { editsuivie_douanes, setEditModal, setsuivie_douanesValidation, setrefresh } from "./suivie_douanesslice";

const Editsuivie_douanes = () => {
  const { editmodal,suivie_douanesValidation } = useAppSelector((state) => state.suivie_douanes);
  const dispatch = useAppDispatch();
  const toggle = () => dispatch(setEditModal());
  const { tempId, formdata } = useAppSelector((state) => state.suivie_douanes);
  const {register,formState: { errors },handleSubmit, reset} = useForm<suivie_douanesType>({
    values: formdata,
  });

  const Editsuivie_douanes: SubmitHandler<suivie_douanesType> = async (data) => {
    console.log(data);
    if (data !== undefined) {

      try {

        const result =  await dispatch(editsuivie_douanes(data));
  ...
  return (
    <>
      <Modal className="modal-bookmark" isOpen={editmodal} toggle={toggle} size="lg">
        ...
        <ModalBody>
          <Form className="form-bookmark needs-validation" onSubmit={handleSubmit(Editsuivie_douanes)}>
            <Row className="g-2">
                                    <FormGroup>
            ...
            <Button color="secondary" className="me-1" type="submit" onClick={() => dispatch(setsuivie_douanesValidation(true))}>Modifier</Button>
...
export default Editsuivie_douanes;
