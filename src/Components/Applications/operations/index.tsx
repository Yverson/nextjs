...
import { FilteroperationsData, operationsListTableDataColumn } from "./columns";
import ReactPaginate from "react-paginate";
import "./styles.css";
import { useDispatch } from "react-redux";
import { fetchData, setSearcParam, setrefresh } from "./operationsslice";
...
const CoperationsListContainer = () => {
  const dispatch = useDispatch();
  const [filterText, setFilterText] = useState("");
  const { refresh, searcParam, operationsFilter } = useAppSelector(
        ...
        <Col sm="12">
          <Card>
            <CardBody>
                ...
                <Collapse isOpen={operationsFilter}>
                  <Card className="shadow-none">
                      ...
                      <Row className="row-cols-xl-5 row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-2 g-3">
                        {FilteroperationsData.map((item, index) => (
                          <Col key={index}>
                            <Input
                              type="select"
                              onChange={(e) =>
                ...
                <div className="table-responsive">
                  <DataTable
                    ...
                    columns={operationsListTableDataColumn}
                    striped
                    highlightOnHover
                    subHeader
...
export default CoperationsListContainer;
